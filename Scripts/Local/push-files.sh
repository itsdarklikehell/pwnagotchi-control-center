...
FILES=$(
    whiptail --title "Check list example" --checklist \
        ...
        "Handshakes" "Push handshakes from $ROOT_MOUNT_DIR/$HANDSHAKE_DIR to $BACKUP_DIR/Handshakes" ON \
        "Main Config" "Push main config.toml from /etc/pwnagotchi/config.toml to $BACKUP_DIR/Configs" ON \
        "Plugin Files" "Push plugin files from $CUSTOM_PLUGIN_DIR to $BACKUP_DIR/Plugins" ON \
        "Plugin Configs" "Push plugin configs from $CUSTOM_PLUGIN_DIR to $BACKUP_DIR/Configs" ON \
        3>&1 1>&2 2>&3
)
exitstatus=$?
if [ $exitstatus = 0 ]; then
...
if [[ "$FILES" =~ "Main Config" ]]; then
    echo "Pushing main config.toml from $BACKUP_DIR/Configs/config.toml to /etc/pwnagotchi/config.toml and /boot/config.toml"
